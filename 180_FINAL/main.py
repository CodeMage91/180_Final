...
def all_users():
    ...
    _chat=None
    if user_id:
        _chat = db.session.execute(
            text(f"SELECT * FROM chat WHERE user1={login.user_id} OR user2={login.user_id}")).mappings().fetchall()
    conversation = None
    ...
    _chat = db.session.execute(
        text(f"SELECT * FROM chat WHERE user1={user_id} OR user2={user_id}")).mappings().fetchall()
    conversation = None
    if request.form:
        if "whichchat" in request.form:
            conversation = db.session.execute(
                text(f"SELECT * FROM message WHERE forchat={request.form["whichchat"]}")).mappings().fetchall()
...
def chat():
    ...
    if chatid==None:
         db.session.execute(text(f"INSERT INTO chat (user1, user2) VALUES ({user_id},{request.form["userid"]})"))
         db.session.commit()
